...
PurchasingManagerNoDemand::PurchasingManagerNoDemand() : PurchasingManager() {}

void PurchasingManagerNoDemand::send_demand_requests__D() {
    for (int i = 0; i < business_connections.size(); i++) {
        business_connections[i]->send_demand_request__D(business_connections[i]->initial_flow__Z_star);
    }
}
